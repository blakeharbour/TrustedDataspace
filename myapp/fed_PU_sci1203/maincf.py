...
from ..viewsmult import updatemultmodel_level


async def main(modelid):
    # 构建存储路径
    log_dir = os.path.join(os.path.dirname(__file__), modelid, 'tensorboard_logs')
    print(log_dir)
    ...
    if data_name == 'minst':
        ...
        # print(train_right_sets)
        # print(test_left_sets)
        # print(test_right_sets)
    ...
    #定义样本数量
    num_sample=0
    num_sample_success=0

    for client in Clients:
        for dtype in data_types:
            ids = client.get_data_ids(is_train=dtype)
            inter_ids = server_client.data_intersection(ids, is_train=dtype)
            if dtype in ("train", "test"):
                num_sample=num_sample+len(ids)
                num_sample_success=num_sample_success+len(inter_ids)

            client.set_dataloader(inter_ids, is_train=dtype)
    sample_dict = {
        "SAMPLE_NUM": num_sample,
        "SAMPLE_NUM_SUCCESS": num_sample_success,
        "modelid":modelid
    }
    from ..views import insertSampleAlignment
    sample_json = json.dumps(sample_dict, ensure_ascii=False)
    print("-----------开始存储样本对齐结果-----------")
    # saveSampleAlignment(sample_json)
    insertSampleAlignment(sample_json)
    ...
    for loss_key in loss_keys:
        ...
        for epoch in range(100):
            ...
            for data1, (data2, target, _) in zip(client.train_loader, server_client.train_loader):
                ...
                print(f"Epoch: {epoch + 1}  Rail端模型output为{output2}")
                print("-------------------------正在加密传输-------------------------")
                print("-------------------------加密传输完成-------------------------")
