...
def compute_best_rbf_kernel_width(dot_prod_matrix, norms_squared, N, M):
    # Ensure norms_squared is a column vector
    norms_squared = norms_squared.reshape(-1, 1)
    # Convert norms_squared to Dask array
    norms_squared_dask = da.from_array(norms_squared, chunks=(9600, 1))
    # If you need the result in memory (be careful, might be large!):
    distance_sqd_matrix = (norms_squared_dask + norms_squared_dask.T - 2 * dot_prod_matrix).compute()
...
def wrapper(dot_prod_matrix, norms_squared, N, M):

    best_width, kernel = compute_best_rbf_kernel_width(dot_prod_matrix, norms_squared, N, M)
...
def KM2_estimate(dot_prod_matrix, norms_squared, N, M):
    KM1, KM2 = wrapper(dot_prod_matrix, norms_squared, N, M)
    return KM2
