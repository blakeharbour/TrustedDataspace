...
from myapp.fed_PU_sci1203.PULoss import PULoss, nnPUSBloss, ImPULoss, ImnnPUSBloss
from myapp.fed_PU_sci1203.client import Client
from myapp.fed_PU_sci1203.data_loader import load_dataset, vertical_split_datasets, vertical_split_testset, load_container_dataset, \
    reconstruct_data, process_data
from myapp.fed_PU_sci1203.evaluator import PUEvaluator
from myapp.fed_PU_sci1203.plot import draw_losses_test_data, draw_precision_recall, draw_error_test_data
from myapp.fed_PU_sci1203.server import Server
from myapp.fed_PU_sci1203.splitNN import SyNet_client, SyNet_server, SyNet_client_coleft, SyNet_client_coright, SyNet_server_co
from myapp.fed_PU_sci1203.utils import compute_prior, resample_data, get_train_with_positive_half, EarlyStopping

import pickle
import os
import json
from torch.utils.tensorboard import SummaryWriter
from myapp.service import TensorBoardService
import webbrowser
...
async def main(modelid):
    ...
    # 构建存储路径
    log_dir = os.path.join(os.path.dirname(__file__), 'modelid', modelid, 'tensorboard_logs')
    print(log_dir)
    ...
    for loss_key in loss_keys:
        ...
        for epoch in range(100):
            ...
            result = []
            client = Clients[0]
            for i, (data1, (data2, target, _)) in enumerate(zip(client.train_loader, server_client.train_loader)):
                ...
                # output2要做成公共的变量，在上一个结果中得到后下一步还能用
                ...
                # 添加TensorBoard记录 - 记录客户端模型的梯度
                # for name, param in client.model.named_parameters():
                #     writer.add_histogram(f'Client Gradients/{name}', param.grad, epoch)
                #
                # # 添加TensorBoard记录 - 记录服务器客户端模型的梯度
                # for name, param in server_client.model.named_parameters():
                #     writer.add_histogram(f'Server-Client Gradients/{name}', param.grad, epoch)
                #
                # # 添加TensorBoard记录 - 记录服务器顶层模型的梯度
                # for name, param in server_client.top_model.named_parameters():
                #     writer.add_histogram(f'Server Top Model Gradients/{name}', param.grad, epoch)
                # 添加TensorBoard记录 - 记录训练损失
                #writer.add_scalar('Train Loss', loss.item(), epoch * len(client.train_loader) + i)
                loss.backward()
                # 打印服务器客户端模型的结构
                ...
                client.update_parameters(client_gradients, learning_rate=0.001)
                # After loss.backward()
                ...
                writer.add_scalar('Client Train Loss', loss.item(), epoch + 1)
                # 保存客户端和服务端模型***********************************888
        ...
        #         print(f"Average Train Loss: {sum(train_loss_) / len(train_loss_)}")
            ...
            print("tensorboard write Train Loss:",train_loss)
            ...
            # 使用 SummaryWriter 记录 precision、recall、error 等信息
            writer.add_scalar('Precision', result[0], epoch + 1)
            writer.add_scalar('Recall', result[1], epoch + 1)
            writer.add_scalar('Error', result[2], epoch + 1)
            print("tensorboard write Train result:", result[0],result[1],result[2])
            ...
            if (epoch + 1) == 100:
                ...
                level_dict = {
                    ...
                    "model_url": "/home/port/hotel/myapp/fed_PU_sci1203/result/nnPU/"
                }
    ...
    writer.close()
    await tensorboard_service.run_tensorboard()

    # 手动检查 TensorBoard 是否启动并获取 URL
    if tensorboard_service.url:
        webbrowser.open(tensorboard_service.url, new=2)
    else:
        print("TensorBoard 启动，但无法获取 URL。请手动检查。")
...
from myapp.views import updatemodel_level